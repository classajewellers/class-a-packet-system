...
--     cad_approval   CAD Approval   (checkpoint; version status is the decision)
--     casting        Casting
...
--     CAD Design inhouse → CAD Approval inhouse → Casting external
--       → Polish/Finish inhouse → Polish/Set inhouse
--
...
-- The casting order is the latest packet_cad_versions row with status = 'approved'.
-- No pointer column on packets. CAD Approval is workshop_stages key cad_approval,
-- the checkpoint before Casting. Approve sets that version to approved and moves
-- the job to Casting. Request Changes returns it to CAD Design. Reject does not advance.
...
--
-- New table packet_cad_versions. RLS matches workshop_roles (167):
...
CREATE TABLE IF NOT EXISTS public.packet_cad_versions (
...
CREATE INDEX IF NOT EXISTS packet_cad_versions_packet_idx
  ON public.packet_cad_versions (tenant_id, packet_id, version_number DESC);

ALTER TABLE public.packet_cad_versions ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.packet_cad_versions FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS tenant_isolation ON public.packet_cad_versions;
CREATE POLICY tenant_isolation ON public.packet_cad_versions
  FOR ALL
  USING (tenant_id = public.current_tenant_id())
  WITH CHECK (tenant_id = public.current_tenant_id());

COMMENT ON TABLE public.packet_cad_versions IS
...
BEGIN
  ...
  FROM (
    VALUES
      ...
      ('cad_approval',  'CAD Approval',  2),
      ('casting',       'Casting',       3),
      ('polish_finish', 'Polish/Finish', 4),
      ('polish_set',    'Polish/Set',    5)
  ) AS s(key, label, ord)
  WHERE NOT EXISTS (
  ...
  SELECT p_tenant,
         ...
         '[
            ...
            {"name":"CAD Approval","location":"inhouse"},
