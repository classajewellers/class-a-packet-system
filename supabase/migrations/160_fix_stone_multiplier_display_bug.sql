-- Fixes calculate_price()'s stone_multiplier diagnostic field always
-- reporting null/0.00 for every quote-builder item, confirmed 2026-09-23.
--
-- Root cause: the p_stone_rows (multi-row) loop correctly looks up each
-- row's own multiplier into a per-row temp variable (v_row_stone_mult) and
-- correctly uses it to accumulate the real dollar total, but that temp
-- variable was reset to NULL at the end of every loop iteration and never
-- written back to the single scalar v_stone_mult actually returned in the
-- response JSON. Confirmed this is display-only: v_stone_mult is never
-- used in any further calculation, and the real per-row multiplier was
-- always applied correctly to the dollar total (stone_retail) throughout.
--
-- Fix: when there is exactly one stone row (the overwhelming common case,
-- and the only case with one unambiguous multiplier to show), capture that
-- row's multiplier into v_stone_mult before it's reset. For 2+ stone rows
-- landing in different tiers, there is no single correct multiplier to
-- report — v_stone_mult is deliberately left null, and the frontend uses
-- the already-returned inputs.stone_rows_count to show "Mixed" instead of
-- a misleading number (confirmed with Josh: not a feature to build here,
-- just a non-misleading fallback).
--
-- Every other line of this function is byte-identical to migration 134's
-- calculate_price_v4 body — this replaces the live calculate_price
-- function directly (confirmed live under this name, not _v4 — the
-- rename cutover mentioned in 134's own comments already happened).
...
BEGIN
  ...
  IF p_stone_rows IS NOT NULL AND jsonb_array_length(p_stone_rows) > 0 THEN
    ...
    LOOP
      ...
      -- 160: remember the first row's multiplier — only used below if this
      -- turns out to be the ONLY row. Evaluated after the loop, not here,
      -- since at this point in a 2+ row quote we don't yet know the final
      -- row count (an earlier draft of this fix wrongly set v_stone_mult
      -- unconditionally on row 1 and left it stuck at that value for every
      -- later row — caught in pglite verification before shipping).
      IF v_stone_rows_count = 1 THEN
        v_first_stone_mult := v_row_stone_mult;
      END IF;
    ...
    -- Only one unambiguous multiplier exists when there was exactly one
    -- stone row. For 2+ rows landing in different tiers, v_stone_mult is
    -- deliberately left null (see migration header) — the frontend shows
    -- "Mixed" instead of a misleading single number.
    IF v_stone_rows_count = 1 THEN
      v_stone_mult := v_first_stone_mult;
    END IF;
