...
| # | Item | Status | Notes |
|---|---|---|---|
| 2.1 | Supplier Connector Framework — schema + interface | CONFIRMED LIVE ON STAGING | Migration `140_supplier_connector_framework.sql` — confirmed applied and queryable on staging 2026-09-22 (`inventory_suppliers.connector_type`/`connector_credentials`/`connector_last_synced_at` + `supplier_sync_log` all exist). `lib/connectors/types.ts` defines the shared `SupplierConnector` interface. |
| 2.2 | Prana connector + sync-UI wiring | READY FOR TESTING | `lib/connectors/prana.ts` wraps the existing deterministic parser, zero new parsing logic. **Sync-UI wiring now built**: `app/api/inventory/suppliers/[id]/sync/route.ts` (preview — parses via the supplier's connector, records a `supplier_sync_log` row, never writes pricing data) + `.../sync/confirm/route.ts` (commits). Melee pricing is tenant-wide with no per-supplier concept (confirmed by reading the existing confirm route's own comments), so the connector sync commits through the exact same path as the manual Settings → Melee upload — extracted into a shared `lib/meleeImportCommit.ts` used by both `/api/pricing/melee-import/confirm` (refactored, not duplicated) and the new supplier-sync confirm route. `app/inventory/suppliers/page.tsx` gained a Connector selector (None / Prana CSV) on the supplier drawer and a "Sync" button + preview-then-confirm modal on any supplier with a connector configured. Verified: typecheck clean, and the full backend chain (CSV → `pranaConnector.syncFromFile` → `commitMeleeImport` → DB writes) smoke-tested end-to-end against a fake Supabase client — confirms correct tenant-scoped delete-then-insert behaviour. **Not visually verified in a live browser** — same dev-server-won't-bind sandbox limitation hit repeatedly this session. **Also found in passing (not fixed — spawned as a separate task, `task_44209f3a`)**: `app/api/inventory/suppliers/route.ts` (GET) and `.../[id]/route.ts` (PATCH/DELETE) have no `tenant_id` filter at all — a real cross-tenant read/write gap, pre-existing and unrelated to this feature. |
...
## Phase 9 — Tenant isolation & notifications (branch: `security/tenant-scoping-helper`, then `feat/notifications-system` on top of it)
...
| # | Item | Status | Notes |
|---|---|---|---|
| 9.1 | Tenant-isolation audit + `tenantScoped()` helper | **COMPLETE — merged & live on production** | Live audit found ~25 routes with a missing-tenant-filter bug (service-role client, no `.eq('tenant_id', ...)`) — ranging from full cross-tenant data export to cross-tenant UPDATE/DELETE. Root-cause fix: `lib/tenantScoped.ts`, a wrapper making the correct behaviour structurally hard to skip. Critical batch and High batch both verified against real staging data with two tenants — every table PASSED. Also closed several staging-only schema drifts found while verifying (`inventory_movements` real schema, `packet_activity_log`/`workshop_settings`/`packets.blocked_reason`, `inventory_statuses.colour`). Merged to `main` 2026-09-22 (commit `38327e7`), Vercel deploy confirmed `success`. |
| 9.2 | **Separate flag, NOT part of 9.1 — do not fix as part of this PR** | N/A | `profiles` has RLS *enabled* with real policies (migrations 019, 039) — contradicts CLAUDE.md's stated house convention ("RLS disabled on all tables, tenancy enforced in app code"). No later migration found that disables it. Whether this is still true live wasn't confirmed (would need `SELECT relrowsecurity FROM pg_class WHERE relname='profiles'` on both environments). Not a blocker for anything built this session — `profiles` reads/writes go through the service-role client either way, which bypasses RLS regardless of its enabled state — but it's a real inconsistency between documented and actual convention, worth resolving deliberately rather than by accident. |
| 9.3 | Notifications system — Phase 1 (bell/badge, Stripe webhook wiring) | **COMPLETE — merged & live on production, migration 150 applied to production** | New feature (branch `feat/notifications-system`, on top of 9.1's branch since it needs `tenantScoped()`). Found while fixing 9.1: `notifications` table was referenced in 3 places (`quotes/[id]` DELETE, 2x `stripe/webhook` inserts) but never existed anywhere — confirmed via a direct production column-list query returning zero rows. Investigated all 3 references: no UI ever read from this table, and both webhook inserts were fire-and-forget (error never checked) — genuinely half-built (write side coded with real intent — "flag deposit-paid-but-packet-creation-failed for manual handling" — but no table, no display surface). Decision: build it for real rather than leave broken or rip out. Migration `150_notifications_system.sql` — `notifications` table, `user_id` nullable by design (NULL = broadcast to managers/admins, set = targeted to one person), generic `link_type`/`link_id` pointer (not per-type FK columns). API: `GET /api/notifications` (own + visible broadcasts + unread_count), `PATCH /api/notifications/[id]` (mark one read), `PATCH /api/notifications/mark-all-read` — all three use `requireAuth()` (session-verified identity, never a client header) since this is genuinely per-user, not just per-tenant. UI: `components/NotificationBell.tsx`, wired into `components/TopBar.tsx` — bell icon, red unread-count badge, dropdown panel, 60s poll (no realtime infra exists yet). Both Stripe webhook alert points (`app/api/stripe/webhook/route.ts`) now insert real rows as broadcasts (`user_id: null`) — removed the fragile `quote.assigned_to` (free-text) → `profiles.full_name` match that used to feed a `user_id` nobody used correctly anyway. pglite-verified: idempotent, broadcast-vs-targeted visibility logic correct, unread count drops after mark-as-read. **Migration 150 applied to staging and re-verified against real staging profiles (2026-09-22)**: 12/12 checks pass — manager sees only the broadcast (not the staff-targeted row, not tenant B's), staff sees only their own targeted row, unread_count correct for both roles, mark-as-read and mark-all-read both work and are correctly scoped (staff cannot mark a broadcast read since it's outside their visibility filter; mark-all-read never touches another tenant's row). Typecheck clean throughout. **Ready for final review and merge.** |
| 9.4 | Notifications system — Phase 2 (quote follow-up reminders) | BLOCKED — awaiting Josh's decision | Two open design questions before building: (1) no cron/scheduled-job infrastructure exists in this repo at all (no `vercel.json`, no `app/api/cron/*`) — needs either new Vercel Cron infra + a `CRON_SECRET`-style auth pattern, or an on-demand-computed-at-login approach instead (simpler, no new infra, but no true "appeared overnight" moment). (2) `quotes.assigned_to` is free text (a staff name), not a `uuid` FK to `profiles` — targeting a follow-up reminder at "the assigned person" requires the same fragile `full_name` match already found broken once in the Stripe webhook (now removed from Phase 1). Holding until Josh weighs in on both. |
| 9.5 | **KNOWN BUG, not fixed** — Settings → Pricing stone-tier UI writes to the wrong columns | NOT STARTED | `app/settings/pricing/page.tsx` (and its API routes, `app/api/pricing-hub/component-rules/{route,[id]/route}.ts`) read/write `carat_min`/`carat_max` on `pricing_component_rules`. `calculate_price()` has read `cost_min`/`cost_max` instead since migration `133_calculate_price_cost_based_stone_tiers.sql`. Net effect: editing "Natural stone tiers" or "Lab stone tiers" in Settings today has **zero effect** on actual pricing — the UI is editing dead columns. Confirmed directly against staging (2026-09-22): staging's `cost_min`/`cost_max` rows for `natural_stone` already match Josh's confirmed-correct tier table exactly (seeded by migration 135), so pricing itself is currently correct — it's specifically the Settings UI that's broken for making future edits. Flagged per Josh's explicit instruction, not fixed yet. |
...
- Phase 9.4: cron vs. on-demand for quote follow-up reminders, and whether to fix `quotes.assigned_to`'s text-vs-uuid mismatch before targeting reminders at a specific person.
- Phase 9.2: is `profiles` RLS enabled-with-policies intentional (an exception to the documented convention) or an oversight to fix?
