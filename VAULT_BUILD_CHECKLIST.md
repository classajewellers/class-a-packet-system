...
| 2.3 | Reporting engine — dimensions/metrics/filter abstraction | IN PROGRESS | `lib/reporting/engine.ts` built (groupByKey/priorPeriodRange/percentChange/topN/sumBy/average). **Sales and Orders sections migrated + live** (`lib/reporting/reports/{sales,orders}.ts`) — both verified byte-for-byte identical against the original hardcoded implementation on real staging data (populated range + zero-rows edge case) before swapping into the live route. Remaining 4 sections (workshop/quotes/customers/staff) + the inventory stub still on the original hardcoded path — migrate incrementally, same verify-then-swap method. |
