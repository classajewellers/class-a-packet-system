...
| 2.3 | Reporting engine — dimensions/metrics/filter abstraction | IN PROGRESS | `lib/reporting/engine.ts` built (groupByKey/priorPeriodRange/percentChange/topN/sumBy/average). **Sales, Orders, Workshop, and Quotes sections migrated + live** (`lib/reporting/reports/{sales,orders,workshop,quotes}.ts`) — each verified byte-for-byte identical against the original hardcoded implementation on real staging data before swapping into the live route. Quotes' multi-counter `byStaff` (total/won/lost/converted) deliberately kept as a local reducer rather than forced through the engine's single-metric `groupByKey`. Remaining 2 sections (customers/staff) + the inventory stub still on the original hardcoded path. |
