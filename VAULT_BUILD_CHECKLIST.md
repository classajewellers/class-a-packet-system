...
| 2.3 | Reporting engine — dimensions/metrics/filter abstraction | IN PROGRESS | `lib/reporting/engine.ts` built (groupByKey/priorPeriodRange/percentChange/topN/sumBy/average). **Sales, Orders, Workshop, Quotes, and Customers sections migrated + live** (`lib/reporting/reports/{sales,orders,workshop,quotes,customers}.ts`) — each verified byte-for-byte identical against the original hardcoded implementation on real staging data before swapping into the live route. Customers' per-email roll-up (first/last visit, total spend across ALL packets ever, unfiltered by date at query time) is a running accumulator, not a one-pass count+sum, so it's a local Map-based reducer like Quotes' byStaff. Only the **staff** section + the inventory stub remain on the original hardcoded path. |
