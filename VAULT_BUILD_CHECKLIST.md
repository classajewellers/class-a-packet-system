...
**IMPORTANT — branch divergence note:** this phase's branches were created off `main`, not off `feat/reporting-engine-sales-migration` (where this checklist file otherwise lives). The two branch lineages need reconciling at merge time — neither has the other's commits yet. Flagging explicitly so this isn't missed.

| # | Item | Status | Notes |
|---|---|---|---|
| 9.1 | Tenant-isolation audit + `tenantScoped()` helper | READY FOR TESTING | Live audit found ~25 routes with a missing-tenant-filter bug (service-role client, no `.eq('tenant_id', ...)`) — ranging from full cross-tenant data export to cross-tenant UPDATE/DELETE. Root-cause fix: `lib/tenantScoped.ts`, a wrapper making the correct behaviour structurally hard to skip. Critical batch (9 routes/files, cross-tenant SELECT) and High batch (9 routes/files, cross-tenant UPDATE/DELETE) both built and verified against real staging data with two tenants — every table PASSES cross-tenant isolation checks. Also closed several staging-only schema drifts found while verifying (`inventory_movements` real schema, `packet_activity_log`/`workshop_settings`/`packets.blocked_reason` — migration 072 never applied to staging, `inventory_statuses.colour` — genuine missing feature, not drift, real production values used). `notifications` table referenced by one of the High-batch fixes (`quotes/[id]/route.ts`) was found to not exist anywhere (staging OR production, confirmed) — see 9.3, this became its own feature. **4 commits on `security/tenant-scoping-helper`, kept as one PR per Josh's decision — not split into separate Critical/High PRs.** Ready for final review and merge. |
