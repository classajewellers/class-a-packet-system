...
| 2.3 | Reporting engine — dimensions/metrics/filter abstraction | IN PROGRESS | `lib/reporting/engine.ts` built (groupByKey/priorPeriodRange/percentChange/topN/sumBy/average). **Sales, Orders, and Workshop sections migrated + live** (`lib/reporting/reports/{sales,orders,workshop}.ts`) — each verified byte-for-byte identical against the original hardcoded implementation on real staging data before swapping into the live route (Workshop also verified against a simulated table-not-found case, since `workshop_jobs` exists but is empty on staging so that branch couldn't be exercised for real). Remaining 3 sections (quotes/customers/staff) + the inventory stub still on the original hardcoded path — migrate incrementally, same verify-then-swap method. |
