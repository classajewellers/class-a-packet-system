...
export default function MajorJobsPage() {
  ...
  const fetchJobs = useCallback(async () => {
    if (!hydrated) return;
    try {
      const res = await fetch("/api/workshop/job-lists?type=major", {
        cache: "no-store",
  ...
  useEffect(() => {
    if (hydrated && user && !isManager) {
      router.replace("/workshop");
    }
  }, [hydrated, user, isManager, router]);
  ...
  if (filterManufacture) {
    filtered = filtered.filter((j) => j.manufacture_type === filterManufacture);
  }
  if (filterSupplier) {
    filtered = filtered.filter((j) => j.workshop_supplier === filterSupplier);
  }
    ...
    <div style={{ fontFamily: "Inter, sans-serif", padding: "24px 32px", paddingBottom: 40, backgroundColor: "#F9FAFB", minHeight: "100vh" }}>
      ...
      >
        ...
        <select
          ...
          onChange={(e) => setFilterManufacture(e.target.value)}
          style={{
            padding: "8px 12px",
            border: "1px solid #E8E8F0",
            borderRadius: 8,
        ...
        >
          <option value="">All Manufacture Types</option>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, fontFamily: "Inter, sans-serif" }}>
            <thead>
              <tr style={{ background: "#F9FAFB" }}>
                {["Date Taken", "Surname", "Description", "Category", "Staff", "Due Date", "Manufacture Type", "Supplier", "Status"].map((col) => (
                  <th
                    key={col}
                    style={{
                      padding: "10px 12px",
              ...
              {filtered.map((job) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Description */}
                    <td style={{ padding: "10px 12px", color: "#374151", maxWidth: 220, verticalAlign: "top" }}>
