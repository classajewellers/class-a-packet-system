...
export interface CreatePacketOptions {
  /**
   * Skip the order-confirmation SMS (Zapier/Podium) and claim-slip URL
   * generation entirely. Used for packets auto-created from a paid quote,
   * pending manager review — nothing customer-facing beyond the dedicated
   * pending-approval email should fire before a human has looked at it.
   */
  skipClaimSlip?: boolean;
  /**
   * Marks the packet as needing manager approval before it can progress
   * through the workshop pipeline. See migration 130.
   */
  pendingCustomerApproval?: boolean;
}
...
/**
 * Core packet-creation logic, extracted from app/api/submit/route.ts so it
 * can be called both from that route (manual, staff-submitted form) and from
 * the Stripe webhook's auto-order-creation path (lib/generatePaymentLink.ts's
 * caller, app/api/stripe/webhook/route.ts) without duplicating ~250 lines of
 * reference-number/customer-upsert/quote-conversion/charm-PO logic.
 */
export async function createPacket(
  ...
  try {
    ...
    return { packet: null, errors: { reference: msg } };
  }
  ...
  const repairTrackerNumber =
    formData.packet_type === "repair" ? generateRepairTrackerNumber(referenceNumber) : null;
  ...
  if (opts?.pendingCustomerApproval) {
    insertData.pending_customer_approval = true;
  }
  ...
  // ── 3. Resolve tenant + customer BEFORE inserting packet ─────────────────
  ...
  if (formData.customer_email) {
    try {
      ...
      console.warn("[createPacket] Customer upsert failed:", err instanceof Error ? err.message : String(err));
    }
  }
